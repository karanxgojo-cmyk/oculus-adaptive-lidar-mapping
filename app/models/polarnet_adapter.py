"""PolarNet Perception Adapter.
Polar-coordinate voxel representation for autonomous driving point cloud segmentation.
"""
...
class PolarNetAdapter(BasePerceptionAdapter):
    """Adapter for PolarNet (Polar coordinate grid segmentation)."""

    def __init__(self, weights_path: Optional[str] = None):
        super().__init__(name="PolarNet (Polar Ring Grid)", mode="DEMO/SIMULATED")
    ...
    def predict(
        ...
        if ground_truth_labels is not None and len(ground_truth_labels) == len(points):
            ...
            confidences = np.random.uniform(0.90, 0.99, size=len(points)).astype(np.float32)
        else:
